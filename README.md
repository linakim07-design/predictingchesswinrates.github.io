...
실행 코드
<img width="456" height="896" alt="스크린샷 2026-06-09 오후 1 07 09" src="https://github.com/user-attachments/assets/c13457f0-11ff-4e0c-947d-3eb7e2c3eb8c" />
...
그래프를 면밀히 분석해 보면 체스 게임의 흥미로운 통계적 특성을 몇 가지 발견할 수 있다.

1. 흑(Black)이 유독 유리한 오프닝
* **Van't Kruijs Opening**: 전체 상위 10개 오프닝 중 가장 압도적인 경기 수를 기록했다. 하지만 놀랍게도 흑의 승리 빈도가 백보다 훨씬 높게 나타난다. 백이 선공의 이점을 살리지 못하는 대표적인 오프닝임을 데이터가 증명한다.
* **Sicilian Defense (시실리안 디펜스)**: 기본 'Sicilian Defense'와 더불어 'Sicilian Defense: Bowdler Attack' 모두 **흑의 승률이 백을 상회**하거나 대등한 강력한 모습을 보여준다. 현대 체스에서 왜 시실리안 디펜스가 흑의 가장 강력한 방어 무기로 꼽히는지 통계적으로 확인할 수 있다.

2. 백(White)이 확실한 우세를 가져가는 오프닝
* **Scandinavian Defense: Mieses-Kotroc Variation**: 이 오프닝에서는 **백(white)의 승률이 흑(black)을 압도한다. 흑이 초반 대응을 잘못할 경우 백에게 주도권을 완전히 내어줄 확률이 높음을 시사한다.
* **Scotch Game (스코치 게임)**: 전통적으로 백이 중앙을 강하게 압박하는 오프닝답게, 백의 승리 횟수가 흑보다 유의미하게 높게 관찰되었다.

---

V. 한계점 발견: 데이터 불균형(Data Imbalance) 문제

1. MLP 모델은 75%의 정확도를 보였으나, 혼동 행렬을 깊이 분석한 결과 치명적인 한계가 있었다. 약 2만 건의 체스 게임 데이터 중 무승부 비율이 극히 적어, 모델이 실제 무승부 경기를 단 한 번도 맞추지 못하고 모두 패배나 승리로 오분류하는 현상이 발생했다.

2. 기술적 해결 방안: SMOTE 오버샘플링 도입
소수 클래스가 무시되는 현상을 해결하기 위해 SMOTE 기법을 도입했다.

단순 복제가 아닌, K-최근접 이웃 알고리즘을 기반으로 무승부 데이터의 특성을 분석해 새로운 가상 데이터를 합성하는 방식을 사용했다.

이때 모델의 평가 신뢰도를 유지하기 위해, 검증&테스트 데이터는 원본 그대로 두고 오직 훈련 데이터(Train Set)에만 SMOTE를 적용하여 데이터 누수를 방지했다.
...
김민결
이병규
김리나- 김민결 데이터사이언스학부 ki30ms19@gmail.com
- 이병규 데이터사이언스학부 lbg0217@hanyang.ac.kr
- 김리나 경영학부 linakim07@hanyang.ac.kr 
...
김민결 코드구현 영상녹화
이병규 코드구현 데이터셋 처리 
김리나 코드구현 블로그 작성 데이터셋 처리

영상 링크 https://youtu.be/a3lDmRK1dJg
